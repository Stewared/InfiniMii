#!/usr/bin/env python3
"""Build the source-sealed exact-current-output whole-draw extension."""
...
SOURCE = ROOT / "renderer" / "native_current_draw_kernel.c"
...
def build(*, force: bool = False, destination: Path | None = None) -> Path:
    del force
    suffix = sysconfig.get_config_var("EXT_SUFFIX")
    if not isinstance(suffix, str) or not suffix:
        raise RuntimeError("Python did not report an extension-module suffix")
    if destination is None:
        destination = ROOT / "renderer" / f"_native_current_draw{suffix}"
    else:
        destination = destination.resolve()
        if destination.name != f"_native_current_draw{suffix}":
            raise ValueError("custom destination must retain the extension module filename")
        destination.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="ltd-native-current-draw-") as directory:
        temporary = Path(directory)
        built = temporary / destination.name
        source_sha256 = hashlib.sha256(SOURCE.read_bytes()).hexdigest()
        opaque_source_sha256 = hashlib.sha256(OPAQUE_SOURCE.read_bytes()).hexdigest()
        (temporary / "native_current_draw_build.h").write_text(
            f"#define LTD_NATIVE_CURRENT_DRAW_ABI_VERSION {ABI_VERSION}\n"
            f'#define LTD_NATIVE_CURRENT_DRAW_SOURCE_SHA256 "{source_sha256}"\n'
            f'#define LTD_NATIVE_CURRENT_DRAW_OPAQUE_SOURCE_SHA256 "{opaque_source_sha256}"\n',
            encoding="ascii",
            newline="\n",
        )
        if sys.platform == "win32":
            ...
            completed = subprocess.run(
                [
                    compiler, "-shared", "-O3", "-ffp-contract=off", "-fPIC",
                    f'-I{sysconfig.get_paths()["include"]}',
                    f"-I{numpy.get_include()}", f"-I{temporary}",
        ...
        if completed.returncode:
            raise RuntimeError(
                "native current-draw build failed:\n"
